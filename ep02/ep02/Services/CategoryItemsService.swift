...
class CategoryItemsService {
    func fetchCategoryItems(category: String, completion: @escaping (Result<[RecipeItem], Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                let decodedResponse = try JSONDecoder().decode(RecipeItemsResponse.self, from: data)
